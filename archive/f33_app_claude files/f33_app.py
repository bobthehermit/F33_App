...
def load_xwalk(path: str, keys: list[str]) -> tuple[pd.DataFrame, list[str]]:
    ...
    part_cols = [c for c in xw.columns if c not in keys]
    for k in keys:
...
if st.button("Generate F-33 Output", type="primary"):
    ...
    with t1:
        ...
        st.caption("Revenue note: objects 41924 (district→charter flow-through)"
                   " and 11112 (restricted cash) are intentionally unmapped in"
                   " the xwalk — excluded from F-33 per FY24 practice.")
    with t2:
        sw = pd.concat([
